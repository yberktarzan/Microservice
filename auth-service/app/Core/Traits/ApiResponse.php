...
trait ApiResponse
{
    ...
    private function respond(
        ...
        $apiVersionHeader = request()->header('X-Api-Version');
        $configVersion = config('api.version');
        $apiVersion = is_string($apiVersionHeader)
            ? $apiVersionHeader
            : (is_string($configVersion) ? $configVersion : '1.0.0');
        ...
        if ($shouldLog && config('api.log_responses', true)) {
            $logData = [
                'success' => $success,
                'message' => $message,
                'status_code' => $status,
            ...
            app(ApiResponseLogger::class)->log($logData);
        }
